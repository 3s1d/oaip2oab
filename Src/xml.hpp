...
public:
	...
	friend std::ostream & operator<<(std::ostream & str, xml::string const & xstr)
	{
		str << xstr.c_str();
		return str;
	}
...
private:
	...
	static void handle_comment(void *data, const XML_Char *comment);
		...
		parser_(XML_ParserCreate(0))
{
	...
	XML_SetCommentHandler(parser_, handle_comment);
}
...
template<class Handler>
void parser<Handler>::handle_data(void *data, const char *content, int length)
{
	static_cast<Handler*>(data)->handle_data(string(content), length);
}


template<class Handler>
void parser<Handler>::handle_comment(void *data, const XML_Char *comment)
{
	static_cast<Handler*>(data)->handle_comment(comment);
}
